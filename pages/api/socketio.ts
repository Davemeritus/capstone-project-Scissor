...
const ioHandler = (req: NextApiRequest, res: NextApiResponseServerIo) => {
  try {
    ...
    if (!res.socket.server.io) {
      ...
      io.use(async (socket, next) => {
        try {
          ...
          if (!userId || !email || !role) {
            log.warn("Missing authentication data in socket handshake");
            return next(new Error("Authentication failed: Missing data"));
          }

          socket.data.user = { id: userId, email, role };
          log.info("Socket user authenticated:", socket.data.user);
          next();
        } catch (error) {
          const err = error as Error;  // Explicitly cast to Error
          log.error("Socket authentication failed:", err.message);
          next(new Error("Authentication failed"));
        }
      });
      ...
      io.on("connection", (socket) => {
        ...
        socket.on("getDashboardData", async () => {
          log.info("Received getDashboardData request from client:", socket.id);
          try {
            const dashboardData = await userService.getUserStats(socket.data.user.id);
            log.info("Sending dashboard data to client:", socket.id);
            socket.emit("dashboardData", dashboardData);
          } catch (error) {
            const err = error as Error;  // Explicitly cast to Error
            log.error("Error fetching dashboard data:", err.message);
            socket.emit("dashboardError", "Failed to fetch dashboard data");
          }
        });
    ...
    const err = error as Error;  // Explicitly cast to Error
    log.error("Error in ioHandler:", err.message);
    res.status(500).end();
  }
};
